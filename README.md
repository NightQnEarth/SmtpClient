...
## Запуск тестов
Тесты находятся в файле test_smtp.py.<br />
Пример запуска тестов (os Windows10):<br />
...
* --server-port [-s]		- Обязательным именнованным аргументом вводится адрес SMTP-сервера и, 
							...
							(по умолчанию порт равен 465).<br />
...
* --attaches				- Необязательным именнованным параметром принимается строка с путями до файлов, которые 
							вы хотите прикрепить (пути не должны содержать пробелов):<br />
...
* --open-sending [-o]		- Необязательным именнованым флаг-параметром принимается `--open-sending`, в том случае, если требуется,
							чтобы каждый получатель видел других получателей данного письма.<br />
							По умолчанию получатели не видят друг-друга.

* --max-size				- Необязательным именнованным принимается целое число - максимальный размер вложения письма.<br />
							...
							части удовлетворяющие условию и отправлен по частям.<br />
...
* --mail-to [-m]			- Обязательным именнованным аргументом принимается строка, содержащая почтовые адреса получаетелей
							сообщения: `--mail-to receiver.first@gmail.com receiver.second@gmail.com`<br />
...
Далее, пользователю будет предложено ввести пароль для почтового ящика отправителя. Если логин или пароль отправителя были введены неверно, пользователю будет дано 3 попытки на повторную аутентификацию.<br />
