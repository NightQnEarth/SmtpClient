...
* --help [-h] 				- Справка по использованию программы.__
...
* --server-port [-s]		- Обязательным именнованным аргументом вводится адрес SMTP-сервера и, 
							...
							(по умолчанию порт равен 465).__
...
* --attaches				- Необязательным именнованным параметром принимается строка с путями до файлов, которые 
							вы хотите прикрепить (пути не должны содержать пробелов):__
...
* --open-sending [-o]		- Необязательным именнованым флаг-параметром принимается `--open-sending`, в том случае, если требуется,
							чтобы каждый получатель видел других получателей данного письма.__
							По умолчанию получатели не видят друг-друга.

* --max-size				- Необязательным именнованным принимается целое число - максимальный размер вложения письма.__
							...
							части удовлетворяющие условию и отправлен по частям.__
...
* --mail-to [-m]			- Обязательным именнованным аргументом принимается строка, содержащая почтовые адреса получаетелей
							сообщения: `--mail-to receiver.first@gmail.com receiver.second@gmail.com`__
							... или полный путь до файла, в начале каждой строки которого содержится один почтовый адрес получателя:__
							`--mail-to @filename.txt`

## Использование
(ос Windows10)__
Программа запускается в командной строке с обязательными параметрами `sender_email`, `--server-port` и `--mail-to`:__
`C:\SmtpClient>python main.py sender@yandex.ru -s smtp.yandex.ru -m receiver@yandex.ru`__
Далее, пользователю будет предложено ввести пароль для почтового ящика отправителя. Если логин или пароль отправителя были введены неверно, пользователю будет дано 3 попытки на повторную аутентификацию.__
После ввода пароля пользователь в следующей же строке вводит текст письма.__
